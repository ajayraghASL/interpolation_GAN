...
def conv2d(input_, output_dim, hh=3, ww=3, stride_h =1, stride_w=1, stddev=0.02, mean=0.0, padding='SAME', name="conv2d"):
    with tf.variable_scope(name):
        ...
        w = tf.get_variable("w", [hh, ww, input_.shape[-1], output_dim],
...
#         return h2
#         np.savetxt("/home/ajay/work/Sparse CT/cancerimagingarchivedata/Walnut1/intermediate/conv_block.npy",h2)
#         print(np.shape(h2))
...
def deconv_block(input_, phase, filter_size, output_depth, name="deconv_block"):
    with tf.variable_scope(name):
        ...
        output_shape = [ int(input_.shape[0]), int(input_.shape[1]), int(input_.shape[2]), output_depth]
...
#         np.savetxt("/home/ajay/work/Sparse CT/cancerimagingarchivedata/Walnut1/intermediate/deconv_block.npy",h2)
#         print(np.shape(h2))
        return h2
...
#         print('\tdeconv:',deconv.get_shape())
        ...
        h2 = lrelu( bn( conv2d(h1, output_depth, filter_size, filter_size, name="conv2d1"), phase, name="bn2") )
#         print('\th2:',h2.get_shape())
#         np.savetxt("/home/ajay/work/Sparse CT/cancerimagingarchivedata/Walnut1/intermediate/bilinear_resize_deconv.npy",h2)
#         print(np.shape(h2))
        return h2
...
def bilinear_resize_tanh_deconv_block(input_, phase, filter_size, output_depth, name="deconv_block"):
    with tf.variable_scope(name):
        ...
        h1 = lrelu( bn( deconv, phase, name="bn1") )
        h2 = tf.nn.tanh( bn( conv2d(h1, output_depth, filter_size, filter_size, name="conv2d1"), phase, name="bn2") )
#         np.savetxt("/home/ajay/work/Sparse CT/cancerimagingarchivedata/Walnut1/intermediate/bilinear_resize_tanh_deconv.npy",h2)
#         print(np.shape(h2))
        return h2
